...
class EstudianteModelo extends Actualizador implements InicioSesion {
    ...
    // Retorna todos los registros solo con los datos
    // necesarios para la tabla CRUD.
    public function consultarTablaCRUD(&$bd) {

        $registros = array();

        $sql = 'SELECT email, nombre, apellido_pat, apellido_mat FROM t_usuarios WHERE rol = "estudiante"';
        $sentencia = $bd->prepare($sql);

        if ($sentencia->execute() && ($filas = $sentencia->fetchAll(PDO::FETCH_ASSOC))) {
    ...
    public function eliminarPorId($email, &$bd) {
        $sql = 'DELETE FROM t_usuarios WHERE email = ?';
        $sentencia = $bd->prepare($sql);
        $sentencia->bindParam(1, $email, PDO::PARAM_STR);
