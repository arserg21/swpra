    ...
    public static function recuperar($claves) {
        ...
        foreach($claves as $clave) {
            if (array_key_exists($clave, $_POST)) {
                $campos[$clave] = htmlentities($_POST[$clave]);
            } else {
                $campos = array();
                break;
            }
...
final class FILESClass {
    ...
    public static function buscar($claves = array()) {
        ...
        $existenTodos = true;
        foreach($claves as $clave) {
            $existenTodos = $existenTodos && array_key_exists($clave, $_FILES);
            if (!$existenTodos) {
                break;
            }
        }
    ...
    public static function recuperar($claves) {
        ...
        foreach($claves as $clave) {
            if (array_key_exists($clave, $_FILES)) {
                
                $campos[$clave] = $_FILES[$clave];
            } else {
                $campos = array();
                break;
            }
...
final class PaqueteJSON implements JsonSerializable {
