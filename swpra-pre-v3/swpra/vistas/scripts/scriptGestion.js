...
window.addEventListener('load', function(e) {
    ...
    // Eventos para el Modal ver estudiante:
    // Carga los datos del usuario dentro del modal
    // para poder ver los datos.
    const modalVerEstudiante = this.document.getElementById('modalVerEstudiante');
    modalVerEstudiante.addEventListener('show.bs.modal', function(e) {

        const notificaciones = document.getElementById('notificaciones');
        
        // Recuperar el elemento <a> que origino el evento:
        const btnFalso = e.relatedTarget;

        // Extraer sus parametros:
        const queryString = new URLSearchParams(btnFalso.search);
        ...
        console.log('email u p modalVer: ' + email);
        ...
        realizarPeticion('CRUDEstudiantesControlador.php', opciones)
            .then(response => {
                ...
                e.target.querySelector('#nombreEstudianteVer').innerHTML      = usuario.nombre;
                e.target.querySelector('#paternoEstudianteVer').innerHTML     = usuario.paterno;
                e.target.querySelector('#maternoEstudianteVer').innerHTML     = usuario.materno === null ? '---' : usuario.materno;
                e.target.querySelector('#fnacimientoEstudianteVer').innerHTML = usuario.fnacimiento;
                e.target.querySelector('#sexoEstudianteVer').innerHTML        = usuario.sexo;

                e.target.querySelector('#matriculaEstudianteVer').innerHTML    = usuario.matricula === null ? '---' : usuario.matricula;
                e.target.querySelector('#programaEstudianteVer').innerHTML     = datos.programaReal === null ? '---' : datos.programaReal.nombre;
                e.target.querySelector('#cuatrimestreEstudianteVer').innerHTML = datos.cuatrimestreReal === null ? '---' : datos.cuatrimestreReal.numero;

                e.target.querySelector('#emailEstudianteVer').innerHTML = usuario.email;


            });
    });

    // Eventos para el Modal modificar estudiante:
    // Carga los datos del usuario en los formularios del modal
    // para poder ser modificados.
    const modalModificarEstudiante = this.document.getElementById('modalModificarEstudiante');
    modalModificarEstudiante.addEventListener('show.bs.modal', function(e) {

        const notificaciones = document.getElementById('notificaciones');
        // Recuperar el elemento <a> que origino el evento:
        //const btnFalso = e.relatedTarget;

        //console.log('boton: ' + btnFalso.id);
        ...
        form1.append('accion', 'ver');
        form1.append('email', email);

        console.log('email u p modalModificar: ' + email);

        // Crear el objeto opciones:
        const opcionesForm1 = {method:'post', body:form1};

        //form1.forEach(value => console.log('se enviara: ' + value));

        // Realizar la petición asincrona:
        realizarPeticion('CRUDEstudiantesControlador.php', opcionesForm1)
            .then(response => {

                //console.log('posiblemente se cargen datos en el modal modificar');
                ...
                // Cargar los datos en el modal:
                e.target.querySelector('#nombreEstudianteModificar').value      = usuario.nombre;
                e.target.querySelector('#paternoEstudianteModificar').value     = usuario.paterno;
                e.target.querySelector('#maternoEstudianteModificar').value     = usuario.materno;
                e.target.querySelector('#fnacimientoEstudianteModificar').value = usuario.fnacimiento;
                e.target.querySelectorAll('[name="sexo"]').forEach(radio => {
                ...
                e.target.querySelector('#matriculaEstudianteModificar').value    = usuario.matricula;

                const selectorProgramas = e.target.querySelector('#programaEstudianteModificar');
                ...
                const selectorCuatrimestres = e.target.querySelector('#cuatrimestreEstudianteModificar');
                ...
                e.target.querySelector('#emailEstudianteModificar').innerHTML = usuario.email;
    ...
    modalModificarEstudiante.querySelectorAll('form')
        .forEach(formularioEnModal => {
            formularioEnModal.addEventListener('submit', function(e) {
                ...
                // Obtener el queryString del botón fuente:
                const queryString = new URLSearchParams(window.location.search);
                
                const accion = queryString.get('accion');
                const email =  queryString.get('email');

                // Obtener el formulario e incorporar la acción:
                /*const form2 = new FormData(e.target);
                form2.append('accion', accion);
                form2.append('email',  email);
                form2.append('form',   this.id);*/
                ...
                //const opcionesForm2 = {method:'post', body:form2};

                // Realizar la petición asincrona:
                /*realizarPeticion('CRUDEstudiantesControlador.php', opcionesForm2)
                    .then(response => {
                        console.log(response);
                        return response.datos;
                    })
                    .then(datos => {});*/
                    //modalModificarEstudiante.hide();
    ...
    modalConfirmarAccion.addEventListener('show.bs.modal', function(e) {
        ...
        if (loQueHace === 'modificar') {
            ...
            // Vincular el modal del botón fuente para vincularlo al botón 'abortar'
            // del modal en curso:
            ...
            const btnConfirmar = this.querySelector('#btnConfirmar');
            btnConfirmar.addEventListener('click', modificar);
            console.log('se añadio un listener "modificar"');
            return;
        }
        if (loQueHace === 'eliminar') {
            mensaje.innerHTML = '¿Realmente desea eliminar el registro?';
            // Vincular el mismo modal al botón abortar:
            const btnAbortar = this.querySelector('#btnAbortar');
            btnAbortar.removeAttribute('data-bs-dismiss');
            btnAbortar.removeAttribute('data-bs-target');
            btnAbortar.removeAttribute('data-bs-toggle');
            ...
            console.log('se añadio un listener "eliminar"');
            
        }
        
    });

    // Evento que mueve todos los listener del botón confirmar
    // del modal 'confirmar acción' cuando este se oculta:
    /*modalConfirmarAccion.addEventListener('hide.bs.modal', function() {
        const btnConfirmar = this.querySelector('#btnConfirmar');
        btnConfirmar.removeEventListener('click', eliminar);
        btnConfirmar.removeEventListener('click', modificar);
    });*/
        ...
        <td>
            <div class="d-md-flex justify-content-center" style="min-width: 200px;">
                <div class="d-inline-flex px-2">
                    <a href="Gestion?accion=ver&email=${usuario.email}" class="btn btn-primary" name="btnVer" id="btnVer${index}" data-bs-toggle="modal" data-bs-target="#modalVerEstudiante">
                ...
                <div class="d-inline-flex px-2">
                    <a href="Gestion?accion=modificar&email=${usuario.email}" class="btn btn-primary" name="btnModificar" id="btnModificar${index}" data-bs-toggle="modal" data-bs-target="#modalModificarEstudiante">
