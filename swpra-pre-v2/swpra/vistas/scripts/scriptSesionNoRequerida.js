...
document.addEventListener('readystatechange', function () {
    if (this.readyState === 'interactive') {
        ...
        realizarPeticion('SesionControlador.php', paquete).then(json => {
            //console.log(json);
            if (json.ok) {
                if (json.datos.sesion === 'activa' && (json.datos.rol === 'estudiante' || json.datos.rol === 'profesor')) {
                    window.location.replace('http://localhost/swpra/vistas/Perfil.html?rol='+json.datos.rol);
                }
                if (json.datos.sesion === 'activa' && json.datos.rol === 'administrador') {
                    window.location.replace('http://localhost/swpra/vistas/Gestion.html');
                }
            }
        });
    }
